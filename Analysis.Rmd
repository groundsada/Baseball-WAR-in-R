...
train_data_norm <- as.data.frame(lapply(train_data_numeric, normalize))

# Combine normalized columns with non-numeric columns
train_data_norm <- bind_cols(train_data[, !numeric_cols], train_data_norm)

# Repeat the same steps for test data
numeric_cols_test <- sapply(test_data, is.numeric)
test_data_numeric <- test_data[, numeric_cols_test]

test_data_norm <- as.data.frame(lapply(test_data_numeric, normalize))
test_data_norm <- bind_cols(test_data[, !numeric_cols_test], test_data_norm)

# Convert Top 100 Status to numeric
train_labels <- as.numeric(train_data$Top.100.Status)
test_labels <- as.numeric(test_data$Top.100.Status)


```

Now, let's create the R regression model for predicting "Top 100 Status":
...
start_time <- Sys.time()
lm_model <- lm(Top.100.Status ~ ., data = train_data_norm)
end_time <- Sys.time()
runtime_lm <- end_time - start_time

# Print the summary of the model
summary(lm_model)

# Predict Top 100 Status on test data
predictions_lm <- predict(lm_model, test_data_norm)
...
cat("Runtime (Linear Regression): ", runtime_lm, "\n")
